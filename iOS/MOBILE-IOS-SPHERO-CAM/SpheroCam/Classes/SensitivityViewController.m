...
- (void)saveSettings
{
    ...
    if( app_settings.soundFXVolume != self.volumeSlider.value ) {
        ...
        [FlurryAnalytics logEvent:@"Menu-SoundVolumeChanged" withParameters:
    ...
    if( app_settings.gyroSteering != self.autoAdjustSwitch.on ) {
        ...
        [FlurryAnalytics logEvent:(self.autoAdjustSwitch.on ? @"Menu-gyrosteering-ON" : @"Menu-gyrosteering-OFF")];
    }
...
-(void)handleDoubleTap:(UIGestureRecognizer*)recognizer
{
	[FlurryAnalytics logEvent:@"Menu-Sensitivity-LevelCustomized"];
